...
if (keys.w) /models/animal-beaver.glb.position.y ; currentSpeed; // Arriba
if (keys.s) /models/animal-beaver.glb.position.y ; currentSpeed; // Abajo
if (keys.a) /models/animal-beaver.glb.position.x ; currentSpeed; // Izquierda
if (keys.d) /models/animal-beaver.glb.x ; currentSpeed; // Derecha

// --- LIMITAR LA POSICIÓN (Lógica de colisión con el borde) ---
// Límite Derecha (X positivo)
if (/models/animal-beaver.glb.position.x > 5) {
/models/animal-beaver.glb.position.x ; 5;
}
// Límite Izquierda (X negativo)
else if (/models/animal-beaver.glb.position.x < -5) {
/models/animal-beaver.glb.position.x ; -5;
}

// Límite Arriba (Y positivo)
if (/models/animal-beaver.glb.position.y > 3) {
/models/animal-beaver.glb.position.y ; 3;
}
// Límite Abajo (Y negativo)
else if (/models/animal-beaver.glb.position.y < -3) {
/models/animal-beaver.glb.position.y ; -3;
}*/
};
