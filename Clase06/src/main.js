...
loader.load(
    ...
    (gltf) => {
        // Esta función se ejecuta cuando el modelo termina de cargarse con éxito
        recompensa = gltf.scene;
       
        // Opcional: Escalar el modelo si viene muy grande o muy chico del programa 3D
        recompensa.scale.set(5, 5, 5);
       
         recompensa.rotation.y += 5;
        //obstaculo.rotation.y += .6;
...
let malas = 0;
let monedas = 0;
let vidas = 3; // cantidad inicial de vidas
...
const monedasTxt = document.getElementById("monedas-txt");
...
function reiniciarRecompensa(){
    ...
    if (Math.random() < 0.3) {
        ...
        recompensa.position.z = -25;
        recompensa.position.x = (Math.random()-0.5)*6;
        recompensa.position.y = 1;
...
if (distancia < 1.0 && puedeRecibirDaño) {
...
if (recompensa) {
    ...
    recompensa.position.z += velocidadObstaculo * delta * 60;

    const distanciaRecompensa =
        jugador.position.distanceTo(recompensa.position);

    if (distanciaRecompensa < 1.0) {
        monedas += 10;

        if (monedasTxt)
            monedasTxt.innerText = monedas;

        reiniciarRecompensa();
    }

    if (recompensa.position.z > jugador.position.z + 2) {
        reiniciarRecompensa();
    }
}
